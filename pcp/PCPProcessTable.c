...
static void PCPProcessTable_updateID(Process* process, int pid, int offset) {
   Process_setThreadGroup(process, Metric_instance_u32(PCP_PROC_TGID, pid, offset, 1)); /* __MARKER */
   Process_setParent(process, Metric_instance_u32(PCP_PROC_PPID, pid, offset, 1)); /* __MARKER */
...
static void PCPProcessTable_updateInfo(PCPProcess* pp, int pid, int offset, char* command, size_t commLen) {
   ...
   process->pgrp = Metric_instance_u32(PCP_PROC_PGRP, pid, offset, 0); /* __MARKER */
   process->session = Metric_instance_u32(PCP_PROC_SESSION, pid, offset, 0); /* __MARKER */
   process->tty_nr = Metric_instance_u32(PCP_PROC_TTY, pid, offset, 0);
   process->tpgid = Metric_instance_u32(PCP_PROC_TTYPGRP, pid, offset, 0); /* __MARKER */
   ...
   process->processor = Metric_instance_u32(PCP_PROC_PROCESSOR, pid, offset, 0); /* __MARKER */
...
static void PCPProcessTable_readOomData(PCPProcess* pp, int pid, int offset) {
   pp->oom = Metric_instance_u32(PCP_PROC_OOMSCORE, pid, offset, 0); /* __MARKER */
}
...
static void PCPProcessTable_readAutogroup(PCPProcess* pp, int pid, int offset) {
   ...
   pp->autogroup_nice = Metric_instance_s32(PCP_PROC_AUTOGROUP_NICE, pid, offset, 0); /* __MARKER */
}
...
static void PCPProcessTable_updateUsername(Process* process, int pid, int offset, UsersTable* users) {
   process->st_uid = Metric_instance_u32(PCP_PROC_ID_UID, pid, offset, 0); /* __MARKER */
...
static void PCPProcessTable_updateCmdline(Process* process, int pid, int offset, const char* comm) {
   ...
   int length = strlen(command); /* __MARKER */
   if (command[0] != '(') {
...
static bool PCPProcessTable_updateProcesses(PCPProcessTable* this) {
   ...
   while (Metric_iterate(PCP_PROC_PID, &pid, &offset)) {
      ...
      unsigned int tty_nr = proc->tty_nr; /* __MARKER */
      ...
      if (proc->state == ZOMBIE && !proc->cmdline && command[0]) {
         Process_updateCmdline(proc, command, 0, strlen(command)); /* __MARKER */
